...
void BattleState::tick()
{
    ...
    if (canInteract)
    {
        ...
        else if (currentEnemyHealth <=0 && dynamic_cast<Boss*>(enemy)->gettimeskill()==1){
            setNextState("Win");
            setFinished(true);
            return;
        }
        else if (currentEnemyHealth <=0 && dynamic_cast<Boss*>(enemy)->gettimeskill()==0){
            dynamic_cast<Boss*>(enemy)->settimeskill(1);
            currentEnemyHealth = enemy->getHealth();
        }
    }
}
...
void BattleState::keyPressed(int key)
{
    if (canInteract)
    {
        ...
        if(key=='h' || key=='H')player->setcurrentHealth(player->getHealth());
    }
}
